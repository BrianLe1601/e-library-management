...
import { Search, Lock, Unlock, ShieldCheck, ShieldOff, UserPlus, MoreVertical, Mail, Hash } from 'lucide-react';

const initialUsers = [
  { id: 'USR-001', name: 'Alice Johnson', email: 'alice@email.com', role: 'admin', status: 'active', joined: '2024-01-15', borrows: 12 },
  { id: 'USR-002', name: 'Bob Martinez', email: 'bob@email.com', role: 'user', status: 'active', joined: '2024-02-20', borrows: 7 },
  { id: 'USR-003', name: 'Chloe Davis', email: 'chloe@email.com', role: 'user', status: 'locked', joined: '2024-03-05', borrows: 3 },
  { id: 'USR-004', name: 'David Kim', email: 'david@email.com', role: 'user', status: 'active', joined: '2024-03-18', borrows: 21 },
  { id: 'USR-005', name: 'Emma Wilson', email: 'emma@email.com', role: 'admin', status: 'active', joined: '2024-04-02', borrows: 5 },
  { id: 'USR-006', name: 'Frank Lee', email: 'frank@email.com', role: 'user', status: 'locked', joined: '2024-04-11', borrows: 0 },
  { id: 'USR-007', name: 'Grace Nguyen', email: 'grace@email.com', role: 'user', status: 'active', joined: '2024-05-22', borrows: 9 },
  { id: 'USR-008', name: 'Henry Brown', email: 'henry@email.com', role: 'user', status: 'active', joined: '2024-06-01', borrows: 14 },
  { id: 'USR-009', name: 'Iris Chen', email: 'iris@email.com', role: 'admin', status: 'active', joined: '2024-06-14', borrows: 2 },
  { id: 'USR-010', name: 'Jack Smith', email: 'jack@email.com', role: 'user', status: 'active', joined: '2024-07-03', borrows: 18 },
];
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-white dark:bg-[#111827] rounded-xl border border-slate-200 dark:border-slate-800 p-4">
          ...
          <div className="mt-2 h-1 rounded-full bg-slate-100 dark:bg-slate-700">
            <div className="h-1 rounded-full bg-emerald-500" style={{ width: `${(activeCount / users.length) * 100}%` }} />
          </div>
        </div>
        <div className="bg-white dark:bg-[#111827] rounded-xl border border-slate-200 dark:border-slate-800 p-4">
          <p className="text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wider">Admins</p>
          <p className="text-slate-900 dark:text-white text-2xl mt-1 font-semibold">{adminCount}</p>
          <div className="mt-2 h-1 rounded-full bg-slate-100 dark:bg-slate-700">
            <div className="h-1 rounded-full bg-indigo-500" style={{ width: `${(adminCount / users.length) * 100}%` }} />
          </div>
        </div>
        <div className="bg-white dark:bg-[#111827] rounded-xl border border-slate-200 dark:border-slate-800 p-4">
          ...
          <div className="mt-2 h-1 rounded-full bg-slate-100 dark:bg-slate-700">
            <div className="h-1 rounded-full bg-red-500" style={{ width: `${(lockedCount / users.length) * 100}%` }} />
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-[#111827] rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
                ...
                >
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-slate-800 dark:text-slate-100 text-sm font-medium">{user.name}</p>
                        <p className="text-slate-400 text-xs">Joined {user.joined}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-3.5">
                  ...
                  <td className="px-5 py-3.5">
                    <span className={`flex items-center gap-1.5 w-fit px-2.5 py-1 rounded-full text-xs ${
                      user.status === 'active'
                  ...
                  <td className="px-5 py-3.5">
                    <span className="text-slate-600 dark:text-slate-300 text-sm">{user.borrows}</span>
                  </td>
                  <td className="px-5 py-3.5">
                    <button
                    ...
                    >
                      {user.status === 'locked' ? <Unlock size={12} /> : <Lock size={12} />}
        ...
        <div className="px-5 py-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
          ...
          <div className="flex gap-1">
            {[1, 2, 3].map(p => (
              <button key={p} className={`w-7 h-7 rounded-lg text-xs ${p === 1 ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'}`}>
                {p}
              </button>
            ))}
          </div>
