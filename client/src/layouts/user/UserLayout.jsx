...
import { BookMarked, Bell, Settings, ChevronRight, LayoutDashboard, BookOpen } from "lucide-react";
...
export default function UserLayout() {
  ...
  const fetchUnread = useCallback(async () => {
    if (!user?.id) return;
    try {
      const res  = await userService.getMyNotifications({ page: 1, limit: 1, filter: "unread" });
      // Backend trả total trong data.total hoặc data.data.total
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
            ...
            <div className="hidden lg:flex items-center gap-4 bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl border border-slate-200/60 dark:border-slate-700/50 px-5 py-3.5 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.02)] dark:shadow-[0_4px_20px_rgb(0,0,0,0.2)] transition-all duration-500">
              ...
              <div>
                ...
                <p className="text-sm font-black text-slate-800 dark:text-slate-100 mt-0.5">Personal Dashboard</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="md:w-65 shrink-0">
              ...
              {TABS.map((tab) => {
                ...
                return (
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `flex items-center justify-between w-full px-4 py-3.5 text-sm transition-colors border-b border-gray-100 dark:border-slate-700 last:border-0 ${
                        isActive
                    ...
                    {({ isActive }) => (
                      <>
                        ...
                        <div className="flex items-center gap-1.5">
                          {tab.path === "/user/notifications" && unreadCount > 0 && (
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-4 mt-4">
              ...
              <div className="space-y-2.5">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-gray-900 dark:text-gray-100 text-xs truncate max-w-[160px]">{user?.email || "—"}</span>
                </div>
                <div className="flex justify-between text-sm">
            ...
            {/* Quick Stats */}
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-4 mt-4">
              <p className="text-xs text-gray-500 dark:text-gray-400 mb-3 uppercase tracking-wider font-semibold">Quick Stats</p>
              <div className="space-y-2.5">
                <div className="flex justify-between text-sm">
